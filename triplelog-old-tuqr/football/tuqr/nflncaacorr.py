...
@interact
def _(cw=slider(sliderarr,default=1,label='Comps'),yw=slider(sliderarr,default=1,label='Yards'),tw=slider(sliderarr,default=1,label='TDs'),iw=slider(sliderarr,default=1,label='INTs'),minatt=slider(0,250,1,default=50)):
    xc1 = []
    xc2 = []
    xc3 = []
    xc4 = []
    ...
    for i in range(0,len(xc1)):
		allplayers.append([rc[i],round(xc1[i],1),round(xc2[i],1),round(xc3[i],1),round(xc4[i],1)])
    sortteams = sorted(allplayers, key=lambda allplayers_entry: allplayers_entry[1],reverse=True)
    corrtable = [['Correlations: ',round(numpy.corrcoef(xc1,yc)[0,1],2),round(numpy.corrcoef(xc2,yc)[0,1],2),round(numpy.corrcoef(xc3,yc)[0,1],2),round(numpy.corrcoef(xc4,yc)[0,1],2)]]
    ...
    matplotlib.pyplot.scatter(xc3,yc)
