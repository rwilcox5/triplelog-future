...
result = connection.execute("select comp,attempts,yards,tds,interceptions,qbrating, scored, pid,name, ryards, rtds,rushes from games")
...
def calcmyncaaqbrating(x,cw,yw,tw,iw,ryw,rtw):
	if x[1]>0:
		cppoints = x[0]*1./x[1]*100
		ypapoints = x[2]*1./x[1]
		tdpoints = x[3]*1./x[1]*100
		...
		intpoints = 0
	if x[12]>0:
		rypoints = x[10]*1./x[12]
		rtpoints = x[11]*1./x[12]*100
	else:
		rypoints = 0
		rtpoints = 0
	return cppoints, ypapoints, tdpoints, intpoints, cw*cppoints+ yw*8.4*ypapoints+ tw*3.3*tdpoints-iw*2*intpoints+ryw*8*rypoints+rtw*2*rtpoints
...
for i in range(0,len(x)):
	if x[i][7] in px:
		...
		px.append(x[i][7])
		sx.append([0,0,0,0,0,0,0,x[i][7],x[i][8],0,0,0])
for i in range(0,len(px)):
	for ii in range(0,len(x)):
		if x[ii][7]==px[i]:
			if ii%2 ==1:
				for iii in range(0,len(sx[i])-7):
					sx[i][iii]=sx[i][iii]+x[ii][iii]
				for iii in range(len(sx[i])-3,len(sx[i])):
					sx[i][iii]=sx[i][iii]+x[ii][iii]
			if ii%2 ==0:
				sx[i][6]=sx[i][6]+x[ii][6]
				sx[i][9]=sx[i][9]+1
@interact
def _(cw=slider(0,1,default=1),yw=slider(0,1,default=1),tw=slider(0,1,default=1),iw=slider(0,1,default=1),ryw=slider(0,1,default=1),rtw=slider(0,1,default=1),minatt=slider(0,50,default=50)):
	xc1 = []
	xc2 = []
	xc3 = []
	xc4 = []
	...
	for i in range(0,len(sx)):
		if sx[i][1]>=minatt:
			...
			xc4.append(calcmyncaaqbrating(sx[i],cw,yw,tw,iw,ryw,rtw)[4])
			if sx[i][9]>0:
	...
	print table(sortteams[0:10],header_row=['a','b','c','d','e'])
